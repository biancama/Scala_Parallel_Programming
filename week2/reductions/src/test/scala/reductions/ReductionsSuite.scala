...
class ReductionsSuite {
  ...
  @Test def `upsweepSequential should correctly handle the chunk 1 until 4 of an array of 4 elements` = {
    val res = upsweepSequential(Array[Float](0f, 1f, 8f, 9f), 1, 4)
    assertEquals(4f, res, 0)
  }


  @Test def `downsweepSequential should correctly handle a 4 element array when the starting angle is zero` = {
    val output = new Array[Float](4)
  ...
  @Test def `parLineOfSight should correctly handle an array of size 4` = {
    val output = new Array[Float](6)
    parLineOfSight(Array[Float](0f, 1f, 8f, 9f, 13f, 23f), output, 1)
    assertEquals(List(0f, 1f, 4f, 4f, 4f, 4.6f), output.toList)
  }
  ...
  @Test def `parBalance should work for long strings` = {
    def check(input: String, expected: Boolean) =
